...
# Tests that are invoked more than once, each with different arguments. Test
# discovery (below) finds each file once; these entries declare its extra
# invocations. Order within a value is not significant.
TEST_PARAMS = {
    'feature_segwit.py': ['--v2transport', '--v1transport'],
    'p2p_bip434_feature.py': ['', '--v2transport'],
    'p2p_block_sync.py': ['--v1transport', '--v2transport'],
    'p2p_compactblocks_hb.py': ['--v1transport', '--v2transport'],
    'p2p_disconnect_ban.py': ['--v1transport', '--v2transport'],
    'p2p_handshake.py': ['', '--v2transport'],
    'p2p_ibd_stalling.py': ['--v1transport', '--v2transport'],
    'p2p_invalid_block.py': ['--v1transport', '--v2transport'],
    'p2p_invalid_tx.py': ['--v1transport', '--v2transport'],
    'p2p_leak_tx.py': ['--v1transport', '--v2transport'],
    'p2p_net_deadlock.py': ['--v1transport', '--v2transport'],
    'p2p_node_network_limited.py': ['--v1transport', '--v2transport'],
    'p2p_timeouts.py': ['--v1transport', '--v2transport'],
    'rpc_bind.py': ['--ipv4', '--ipv6', '--nonloopback'],
    'rpc_blockchain.py': ['--v1transport', '--v2transport'],
    'rpc_deriveaddresses.py': ['', '--usecli'],
    'rpc_net.py': ['--v1transport', '--v2transport'],
    'rpc_setban.py': ['--v1transport', '--v2transport'],
    'wallet_createwallet.py': ['--usecli', ''],
    'wallet_multiwallet.py': ['', '--usecli'],
    'wallet_txn_clone.py': ['', '--segwit', '--mineblock'],
    'wallet_txn_doublespend.py': ['--mineblock', ''],
}

# The longest-running default test. Under parallel scheduling, only the longest
# test's start time materially bounds total runtime, so it is scheduled first;
# the order of the remaining tests does not need to be maintained by hand.
LONGEST_BASE_SCRIPT = 'feature_taproot.py'

# A functional test is any *.py in this directory whose name starts with one of
# these prefixes (also enforced by check_script_prefixes()).
TEST_NAME_PREFIX = re.compile("^(example|feature|interface|mempool|mining|p2p|rpc|wallet|tool)_")


def get_base_scripts(functional_dir):
    """Discover the default (non-extended) test scripts from the test directory.

    Every prefixed *.py is a base test except the extended-only scripts; files in
    TEST_PARAMS expand to their several invocations, and the longest test is run
    first. Adding a test is just adding the file - no hand-maintained list."""
    discovered = sorted(
        name for name in os.listdir(functional_dir)
        if name.endswith('.py')
    ...
    for name in discovered:
        for params in TEST_PARAMS.get(name, ['']):
            scripts.append(f"{name} {params}".strip())
    if LONGEST_BASE_SCRIPT in scripts:
        scripts.remove(LONGEST_BASE_SCRIPT)
        scripts.insert(0, LONGEST_BASE_SCRIPT)
    return scripts


BASE_SCRIPTS = get_base_scripts(os.path.dirname(os.path.abspath(__file__)))
...
def check_script_prefixes():
    ...
    bad_script_names = [script for script in ALL_SCRIPTS if TEST_NAME_PREFIX.match(script) is None]
